<?php
/**
 * 🔄 CHANNEL hoerbuch BIDIREKTIONALE SYNCHRONISATION (MadelineProto)
 * ================================================================
 * 
 * Synchronisiert Chat-Nachrichten zwischen:
 * - Telegram-Chat (@WeltenbibliothekHoerbuch) ↔ Flutter-App (Firestore)
 * 
 * Features:
...
// Telegram Chat
$CHAT_USERNAME = '@WeltenbibliothekHoerbuch';
...
$FTP_PASS = 'Jolene2305';
$FTP_REMOTE_PATH = '/audios/';  // Chat-Ordner im FTP
$HTTP_BASE_URL = "http://{$FTP_HOST}:8080/audios";  // HTTP-URL für Chat-Ordner

// Firebase
$FIREBASE_SDK = '/opt/flutter/firebase-admin-sdk.json';
$FIRESTORE_COLLECTION = 'hoerbuch_messages';
...
echo "\n";
echo "╔════════════════════════════════════════════════════════════╗\n";
echo "║  🔄 CHANNEL hoerbuch BIDIREKTIONALE SYNCHRONISATION         ║\n";
...
function saveToFirestore($data) {
    ...
    // Escape data for JSON - handle special characters properly
...
PYTHON;
    ...
    if (strpos($output, 'OK') === false) {
        echo "      ⚠️  Firestore Debug: " . substr($output, 0, 200) . "
";
    }
...
while (true) {
    ...
    try {
        ...
        foreach ($messages as $message) {
            ...
            } elseif (isset($message['from_id']) && isset($message['from_id']['user_id'])) {
            ...
            // Extract text from message or caption (for media messages)
            $text = '';
            if (isset($message['message']) && !empty($message['message'])) {
                $text = $message['message'];
            } elseif (isset($message['media']['caption']) && !empty($message['media']['caption'])) {
