<?php
/**
 * ⚡ ECHTZEIT MULTI-CHANNEL SYNC MIT GITHUB INTEGRATION
 * =====================================================
 * 
 * OPTIMIERTER UPLOAD-FLOW:
 * Telegram → MadelineProto → GitHub → Firebase → Flutter App
 *                                  ↓
 *                            FTP (Backup)
 * 
 * VORTEILE:
 * ✅ GitHub CDN für schnelle Downloads
 * ✅ Versionskontrolle aller Medien
 * ✅ Automatische Backups
 * ✅ Raw URLs für direkten Zugriff
 * ✅ FTP als Fallback
 */
...
$GITHUB_REPO_URL = 'https://github.com/manuelb77/weltenbibliothek_media';  // Anpassen!
$GITHUB_RAW_BASE = 'https://raw.githubusercontent.com/manuelb77/weltenbibliothek_media/main';
...
// ⚡ KRITISCH: 1 Sekunde Intervall
$CHECK_INTERVAL_SECONDS = 1;

echo "\n";
echo "╔═══════════════════════════════════════════════════════════════╗\n";
echo "║  ⚡ ECHTZEIT SYNC MIT GITHUB INTEGRATION                     ║\n";
echo "║     GitHub CDN + FTP Fallback                                ║\n";
echo "╚═══════════════════════════════════════════════════════════════╝\n\n";
...
Dieses Repository enthält alle Medien aus den Telegram-Kanälen.
...
Alle Medien sind über Raw-URLs verfügbar:

https://raw.githubusercontent.com/manuelb77/weltenbibliothek_media/main/media/chat/example.jpg
README;
        ...
        echo "⚠️  WICHTIG: Verbinde mit GitHub:\n";
        echo "   git remote add origin https://github.com/manuelb77/weltenbibliothek_media.git\n";
...
echo "⚡ Starte ECHTZEIT Sync mit GitHub Integration...\n";
echo "   📍 Kanäle: " . count($CHANNELS) . "\n";
echo "   ⏱️  Intervall: {$CHECK_INTERVAL_SECONDS} Sekunde\n";
echo "   🎯 Upload: GitHub (Primary) + FTP (Fallback)\n\n";
...
while (true) {
    ...
    foreach ($CHANNELS as $channel) {
        ...
        try {
            ...
            if ($newCount > 0) {
                echo "   ✅ {$channel['name']}: {$newCount} neue Nachrichten\n";
            }
        ...
        try {
            ...
            foreach ($appMessages as $msg) {
                ...
                if ($sentId) {
                    ...
                    echo "   📤 {$channel['name']}: App → Telegram\n";
                }
            }
